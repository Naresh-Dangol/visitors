...
<table class="table table-responsive datatables" id="visitors-table">
    ...
    @foreach($visitors as $index=>$visitor)
        <tr>
            ...
            @if(Auth::user()->user_role != 'admin')
             <td><a href="{{url('visitor-notification')}}" class="btn btn-success">Send</button></td>
             @endif
            <td><button data-visitor-detail-id="{!! $visitor->detailsId !!}" id="status" class="btn btn-primary" style="width: 80%">
                {!! ($visitor->status == "yes")?'Satisfied':'Unsatisfied' !!} </button> 
                @if(Auth::user()->user_role != 'admin')
                <a href="{{route('visitors.edit_purpose',$visitor->id)}}" title="change status"><i class="fa fa-plus"></i></a>
                @endif
            </td>
            @if(Auth::user()->user_role != 'admin')
            <td>
                ...
                <div class='btn-group'>
                    ...
                    <a href="{!! route('visitors.add_purpose', [$visitor->id]) !!}" class='btn btn-default btn-xs' title="Add Status"><i class="glyphicon glyphicon-plus"></i></a>
                </div>
                {!! Form::close() !!}
            </td>
            @endif
        ...
        <!-- Modal -->
<div class="modal fade satisfaction_modal" id="satisfaction_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
